...
#   CHECKPOINT_FILE CODEX_SANDBOX CODEX_MODEL CODEX_EXTRA_ARGS
...
CHECKPOINT_FILE="${CHECKPOINT_FILE:-$STATE_DIR/${TASK}.supervisor-state.json}"
...
process_state(){
  ps -o stat= -p "$1" 2>/dev/null | awk 'NR == 1 { print substr($1, 1, 1) }'
}

checkpoint(){
  local status="$1" observed_at="$2" pause_started_at="${3:-}" resumed_at="${4:-}"
  local paused_seconds="${5:-0}" process_status="${6:-}" tmp="${CHECKPOINT_FILE}.tmp.$$"
  python3 - "$tmp" "$CHECKPOINT_FILE" "$TASK" "$SUB_PID" "$status" "$observed_at" \
    "$pause_started_at" "$resumed_at" "$paused_seconds" "$process_status" "$LOG" "$LAST_MESSAGE" <<'PY'
import json, os, sys

(tmp, target, task, pid, status, observed_at, pause_started_at, resumed_at,
 paused_seconds, process_status, log, last_message) = sys.argv[1:]
payload = {
    "schema_version": 1,
    "task": task,
    ...
    "pause_started_at_epoch": int(pause_started_at) if pause_started_at else None,
    "resumed_at_epoch": int(resumed_at) if resumed_at else None,
    "paused_seconds_total": int(paused_seconds),
    "process_state": process_status or None,
    "log": log,
...
launch_agent(){
  ...
  if [ -n "$SUB_PID" ] && kill -0 "$SUB_PID" 2>/dev/null; then
    ...
    LAST_RESUME_TS="$LAUNCH_TS"
    checkpoint running "$LAUNCH_TS" "" "" 0 "$(process_state "$SUB_PID")"
...
while true; do
  ...
  # SIGSTOP cannot be trapped. Observe the kernel process state instead and
  # checkpoint transitions. Never send SIGCONT or otherwise interfere with the
  # external memory controller that owns pause/resume decisions.
  now=$(date +%s)
  proc_state="$(process_state "$SUB_PID")"
  if [ "$proc_state" = T ] || [ "$proc_state" = t ]; then
    if [ "$PAUSE_STARTED_TS" -eq 0 ]; then
      PAUSE_STARTED_TS="$now"
      checkpoint paused "$now" "$PAUSE_STARTED_TS" "" "$PAUSED_TOTAL_SECONDS" "$proc_state"
      log "PAUSED: PID=$SUB_PID process_state=$proc_state checkpoint=$CHECKPOINT_FILE"
    fi
    continue
  fi
  if [ "$PAUSE_STARTED_TS" -gt 0 ]; then
    pause_duration=$(( now - PAUSE_STARTED_TS ))
    PAUSED_TOTAL_SECONDS=$(( PAUSED_TOTAL_SECONDS + pause_duration ))
    LAST_RESUME_TS="$now"
    checkpoint running "$now" "$PAUSE_STARTED_TS" "$now" "$PAUSED_TOTAL_SECONDS" "$proc_state"
    log "RESUMED: PID=$SUB_PID paused=${pause_duration}s paused_total=${PAUSED_TOTAL_SECONDS}s checkpoint=$CHECKPOINT_FILE"
    PAUSE_STARTED_TS=0
  fi
  ...
  if [ -f "$LOG" ]; then
    ...
    [ "$LAST_RESUME_TS" -gt "$log_mtime" ] && log_mtime="$LAST_RESUME_TS"
    age=$(( now - log_mtime ))
  else
    age=$(( now - LAST_RESUME_TS ))
  fi
  runtime=$(( now - LAUNCH_TS - PAUSED_TOTAL_SECONDS ))
